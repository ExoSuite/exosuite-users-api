...
class FriendshipsUnitTest extends TestCase
{
    ...
    public function testSendFriendshipRequestWithWrongTarget()
    {
        ...
        $response = $this->post(route('sendFriendshipRequest', ['user' => Uuid::generate()->string]));
    ...
    {
        Passport::actingAs($this->user);
        $response = $this->post(route('acceptFriendship', ['request' => Uuid::generate()->string]));
        $response->assertStatus(Response::HTTP_UNPROCESSABLE_ENTITY);
    }
    ...
    {
        Passport::actingAs($this->user);
        $post_resp = $this->post(route('sendFriendshipRequest', ['user' => $this->user1->id]));
        $response = $this->post(route('acceptFriendship', ['request' => $post_resp->decodeResponseJson('request_id')]));
    ...
    {
        Passport::actingAs($this->user);
        $response = $this->post(route('declineFriendship', ['request' => Uuid::generate()->string]));
        $response->assertStatus(Response::HTTP_UNPROCESSABLE_ENTITY);
    }
    ...
    public function testDeclineFriendshipRequestAsWrongTarget()
    {
        Passport::actingAs($this->user);
        $post_resp = $this->post(route('sendFriendshipRequest', ['user' => $this->user1->id]));
        $response = $this->post(route('declineFriendship', ['request' => $post_resp->decodeResponseJson('request_id')]));
        $response->assertStatus(Response::HTTP_FORBIDDEN);
        $response->assertJson(['message' => "You're not allowed to answer this request"]);
    }
    ...
    {
        Passport::actingAs($this->user);
        $response = $this->get(route('friendList', ['user' => Uuid::generate()->string]));
        $response->assertStatus(Response::HTTP_UNPROCESSABLE_ENTITY);
    }
    ...
    {
        Passport::actingAs($this->user);
        $response = $this->delete(route('deleteFriendship', ['user' => Uuid::generate()->string]));
        $response->assertStatus(Response::HTTP_UNPROCESSABLE_ENTITY);
    }
